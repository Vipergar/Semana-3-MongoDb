...
async def create_student(student: StudentModel = Body(...)):
 ...
 created_student = await db["tripulantes"].find_one({"_id": 
...
async def list_students():
 ...
 @app.get(
 "/{id}", response_description="Get a single student", 
response_model=StudentModel
)
...
@app.put("/{id}", response_description="Update a student", 
response_model=StudentModel)
async def update_student(id: str, student: UpdateStudentModel = Body(...)):
 student = {k: v for k, v in student.dict().items() if v is not None}
 
 if len(student) >= 1:
    update_result = await db["tripulantes"].update_one({"_id": id}, {"$set": 
student})
